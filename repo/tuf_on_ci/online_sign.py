...
def online_sign(verbose: int, push: bool) -> None:
    ...
    * or timestamp is currently not correctly signed
    ...
    valid_ss = repo.is_signed("snapshot") and not repo.is_in_signing_period("snapshot")
    snapshot_updated, _ = repo.do_snapshot(not valid_ss)
    valid_ts = repo.is_signed("timestamp") and not repo.is_in_signing_period("timestamp")
    timestamp_updated, _ = repo.do_timestamp(not valid_ts)
